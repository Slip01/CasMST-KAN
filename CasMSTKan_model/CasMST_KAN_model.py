...
"CasMSTKan"
class CasMSTKanNet(nn.Module):

    def __init__(self, input_dim=1, GCN_hidden_size=512, GCN_hidden_size2=8, MLP_hidden1=128, MLP_hidden2=32,
                 Activation_fc=nn.ReLU(),
                 grid_size=4,spline_order=3,
                 kernel_size=5,
                 scale1=3, scale2=5):
        super(CasMSTKanNet, self).__init__()
        # gat
        self.gat1 = GraphAttentionLayer(in_features=input_dim, out_features=GCN_hidden_size, dropout=0.5, alpha=0.5,
                                        concat=True)
        self.gat2 = GraphAttentionLayer(in_features=GCN_hidden_size, out_features=GCN_hidden_size2, dropout=0.5,
                                        alpha=0.5, concat=True)
        ...
        # Kan
        self.FL = kan.KAN(
            layers_hidden=[192, MLP_hidden1, MLP_hidden2],
            grid_size=7,
            spline_order=5,
            # grid_size=grid_size,
        ...
        self.MSTC = MSTC.MultiScale_TemporalConv(in_channels=64,
                                     kernel_size=7,
        ...
        self.Gated_Fusion = Gated_Fusion.gatedFusion(dim=256)
        self.hlinear = nn.Linear(1024, 8)
        self.Relu_Linear_Attention = Relu_Linear_Attention.LiteMLA(in_channels=128, out_channels=128, scales=(5,7))
    ...
    def forward(self, adjacency, feature, time, interval_popularity):
        ...
        h = torch.reshape(h, [64, 128, 8, 1])
        h = self.Relu_Linear_Attention(h)
        h = torch.reshape(h, [64, 128, 8])

        interval_popularity = torch.reshape(interval_popularity, [config.batch_size, -1, 1, 1])
        interval_popularity = interval_popularity+self.MSTC(interval_popularity)

        interval_popularity = torch.reshape(interval_popularity, [config.batch_size, -1, 1])
        interval_popularity1, _ = self.rnn(interval_popularity)
        interval_popularity=interval_popularity+interval_popularity1
